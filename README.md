...
- `/src`:
  ...
  3. `product_iterator.py`: Определяет `class ProductIterator`.
  Содержит следующие методы:
  ...
  4. `smartphone.py` - определяет `class Smartphone`.
  Содержит следующие методы:
  - `__init__()` - метод для инициализации экземпляра класса "Смартфоны" (Smartphone);
  - `__add__()` - метод возвращает сумму произведений цены на количество.
  5. `lawngrass.py` - определяет `class LawnGrass`.
  Содержит следующие методы:
  - `__init__()` - метод для инициализации экземпляра класса "Зеленая трава" (LawnGrass);
  - `__add__()` - метод возвращает сумму произведений цены на количество
  6. `utils.py`: Вспомогательные функции для чтения и обработки данных JSON.
...
src\category.py                      39      2    95%
src\lawngrass.py                     11      3    73%
src\product.py                       43     10    77%
src\products_iterator.py             14      0   100%
src\smartphone.py                    12      3    75%
